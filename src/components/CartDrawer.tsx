...
import { Trash2, X, ShoppingBag, Plus, ArrowRight, ShieldCheck, Zap } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

export function CartDrawer() {
  const { isOpen, closeCart, items, removeItem, getTotal, getSubtotal, addItem, getPromoDiscount, getOriginalTotal, getSavings } = useCart();
  ...
  const handleCheckout = () => {
    // Garantir que temos a URL, caso contrário usa o fallback
    const baseUrl = settings?.integration?.checkoutBaseUrl || "https://checkout.gorgpresets.com/pay/gorgpresets";
    const skus = items.map(item => item.product.id).join(",");
    const quantities = items.map(() => 1).join(",");
    const discountValue = getPromoDiscount();
    const finalUrl = `${baseUrl}?skus=${skus}&qs=${quantities}&cur=${currency}&discount=${discountValue}&ref=gorg_drawer`;
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div 
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[200] flex items-center justify-center p-0 overflow-hidden"
        >
